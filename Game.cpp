...
namespace ECE141 {
  ...
  enum class Reasons {tbd, forfeit, badmove, eliminated, moved2, clockExpired};
  ...
  public:
    ...
    RealGame() : Game(), loserColor(PieceColor::blue), reason(Reasons::tbd), step(0),
      slideLen(sqrt(2)), jumpLen(sqrt(2)*3), priorDist(0), priorPiece(nullptr) {
    ...
    void initPieces(Player &aPlayer1, Player &aPlayer2) {
    ...
    bool isValidMove(const Piece &aPiece,const Tile &aSrc,const Tile &aDest) {
      if(isAvailable(aDest)) {
        if(isValidDirection(aPiece, aSrc.location, aDest.location)) {
          priorDist = distance(aSrc.location, aDest.location);
          if(priorDist>slideLen) {
      ...
      if(Reasons::tbd==reason) {
        ...
        const Tile* theDestTile=getTileAt(aDestination);
         if(isValidMove(aPiece, *theSrcTile, *theDestTile)) {
          const_cast<Tile*>(theSrcTile)->piece=(Piece*)nullptr;
          const_cast<Tile*>(theDestTile)->piece=(Piece*)&aPiece;
          
          if(auto theOpponent = jumpedOpponent(aPiece, aDestination)) {
            removePieceAt(theOpponent->location);
          }

          const_cast<Piece&>(aPiece).location=aDestination;
    ...
    //-- USE: visualization of board -------------------------
    void visualizeBoard() {
      const char* theSpacer = "├───┼───┼───┼───┼───┼───┼───┼───┤";
      const char* theBottom = "└───┴───┴───┴───┴───┴───┴───┴───┘";
